 ...
 * 				Mohammed Hassan
 * 				Mohammed Reda
 * @brief External Interrupt Handling Interface.
 *        This interface provides functions to configure external interrupt lines, set triggering modes,
 *        enable/disable interrupts, and set callback functions for the EXTI IRQs.
 * @version 0.3
 * @date 2023-11-8
...
/**
 * @brief Configures the triggering mode for a specified EXTI line.
 * 
 * @param Copy_u8EXTI_Line       EXTI line number to configure.
 * @param Copy_u8TriggeringMode  Triggering mode to set for the EXTI line.
 * @return u8                    Error state: E_OK if successful, E_NOK if an error occurred.
 */
...
/**
 * @brief Sets the trigger mode for a specified EXTI line.
 * 
 * @param Copy_EXTI_Num       EXTI line number to set the trigger mode for.
 * @param Copy_TriggerState   Trigger mode to set for the EXTI line.
 * @return u8                 Error state: E_OK if successful, E_NOK if an error occurred.
 */
 ...
 * 
 * @param Copy_EXTI_Num    EXTI line number to clear the pending flag for.
 * @return u8              Error state: E_OK if successful, E_NOK if an error occurred.
 */
...
/**
 * @brief Gets the pending flag status for a specified EXTI line.
 * 
 * @param Copy_EXTI_Num     EXTI line number to get the pending flag status for.
 * @param Copy_PenValue     Pointer to store the pending flag status.
 * @return u8               Error state: E_OK if successful, E_NOK if an error occurred.
 */
...
/**
 * @brief Enables interrupts for a specified EXTI line.
 * 
 * @param Copy_EXTI_Num    EXTI line number to enable interrupts for.
 * @return u8              Error state: E_OK if successful, E_NOK if an error occurred.
 */
...
/**
 * @brief Disables interrupts for a specified EXTI line.
 * 
 * @param Copy_EXTI_Num    EXTI line number to disable interrupts for.
 * @return u8              Error state: E_OK if successful, E_NOK if an error occurred.
 */
...
/**
 * @brief Sets a callback function for a specified EXTI line.
 * 
 * @param Copy_u8EXTI_Line    EXTI line number to set the callback function for.
