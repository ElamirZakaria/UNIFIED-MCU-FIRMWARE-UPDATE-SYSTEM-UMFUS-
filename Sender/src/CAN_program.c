...
void CAN_voidInit(void)
{
   ...
   // AFIO_voidRemapPeripheralPins (CAN_REMAP);
    ...
    MGPIO_stderrSetPinMode( MGPIO_PIN_A11, MGPIO_MODE_INPUT_FLOATING); //RX
    MGPIO_stderrSetPinMode( MGPIO_PIN_A12, MGPIO_MODE_AF_PUSHPULL_2MHZ); //TX
    /**  exit CAN sleep mode **/
...
u8 CAN_voidGetRxMsg(uint32 local_u8RxFifo, CAN_RxHeaderTypeDef *pRxHeader, uint8 *Local_u8Data)
{
	uint8 u8_ReturnFlag=0;
	static uint8 j=0;
    ...
    Local_u8Data[7] = (uint8)((CAN->FIFOMailBox[local_u8RxFifo].RDHR >> 24) & 0XFF);
		...
		{
			if (i<4)
				Local_u8Data[j] = (uint8)((CAN->FIFOMailBox[local_u8RxFifo].RDLR >>(i*8) ) & 0XFF);
			else
				Local_u8Data[j] = (uint8)((CAN->FIFOMailBox[local_u8RxFifo].RDHR 	>>((i-4)*8)  ) & 0XFF);
			/*Local_u8Data*/
			if(Local_u8Data[j]=='*')
			{
				u8_ReturnFlag=1;
