...
STD_error_t MGPIO_stderrSetPinMode(u8 ARG_u8PinID, u8 ARG_u8PinMode)
{
	...
	if(ARG_u8PinID <= MGPIO_PIN_C13 && ARG_u8PinMode <= MGPIO_MODE_AF_OPENDRAIN_50MHZ)
	{
...
STD_error_t MGPIO_stderrSetPinState(u8 ARG_u8PinID, u8 ARG_u8PinState)
{
	...
	if(ARG_u8PinID <= MGPIO_PIN_C13)
	{
		switch(ARG_u8PinState)
		{
			...
			case MGPIO_TOGGLE: TOGGLE_BIT(MGPIO_pmgpio_tRegisters[portNo]->ODR, pinNo); break ;
