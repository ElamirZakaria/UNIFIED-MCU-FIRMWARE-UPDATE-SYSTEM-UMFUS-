...
int main(void){
	...
	MGPIO_stderrSetPinMode(MGPIO_PIN_B0, MGPIO_MODE_OUTPUT_PUSHPULL_2MHZ);
	...
	while(1)
	{
		...
		if(Distance < 10 )
		{
			...
			HLED_voidLedOn(HLED_PIN_B1);
			MGPIO_stderrSetPinState(MGPIO_PIN_B0,MGPIO_HIGH);
			MSTK_voidBusyWait(200000);
			HLED_voidLedOff(HLED_PIN_B1);
			MGPIO_stderrSetPinState(MGPIO_PIN_B0,MGPIO_LOW);
			MSTK_voidBusyWait(200000);
		}
		else if(Distance<15)
		{
